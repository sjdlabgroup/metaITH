...
#' @usage metaITH_analysis(dna_dendro_list, rna_dendro_list, immune_dendro_list, multi_region_sample_names, rna_expression_matrix)
#' @param dna_dendro_list A text file containing a list of sample files, one file name per line. Each file contains a matrix with variant allele frequency of all the variations in each tumor region and normal.
#' @param rna_dendro_list A text file containing a list of sample files, one file name per line. Each file contains a matrix with expression values (preferably log2(TPM+1)) of all the genes in each tumor region and normal.
#' @param immune_dendro_list A text file containing a list of sample files, one file name per line. Each file contains a matrix with proportion of immune cells (inferred by CIBERSORT) of all immune cell types in each tumor region and normal.
...
#' @example metaITH_analysis(DNA_dendro_list.txt, RNA_dendro_list.txt, Immune_dendro_list.txt, sample_names.txt, RNA_matrix.txt)
#' @export
metaITH_analysis=function(dna_dendro_list, rna_dendro_list, immune_dendro_list, multi_region_sample_names, rna_expression_matrix){
file <- read.table(dna_dendro_list)
...
file <- read.table(rna_dendro_list)
for (m in 1:length(file[,1])) 
{
...
file <- read.table(immune_dendro_list)
for (m in 1:length(file[,1])) 
{
