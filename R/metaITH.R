...
#' @usage snv_heatmaps(dna_phylo_list_file)
#' @param dna_phylo_list_file A text file containing a list of sample files, one file name per line. Each file contains a matrix with variant allele frequency of all the variations in each tumor region and normal.
...
#' @usage dna_phylograms(dna_phylo_list_file)
#' @param dna_phylo_list_file A text file containing a list of sample files, one file name per line. Each file contains a matrix with variant allele frequency of all the variations in each tumor region and normal.
#' @example dna_phylograms(DNA_phylo_list.txt)
#' @export
dna_phylograms=function(dna_phylo_list_file){
file <- read.table(dna_phylo_list_file)
for (m in 1:length(file[,1])) 
{
...
#' @usage rna_phylograms(rna_phylo_list_file)
#' @param rna_phylo_list_file A text file containing a list of sample files, one file name per line. Each file contains a matrix with expression values (preferably log2(TPM+1)) of all the genes in each tumor region and normal.
#' @example rna_phylograms(RNA_phylo_list.txt)
#' @export
rna_phylograms=function(rna_phylo_list_file){
file <- read.table(rna_phylo_list_file)
for (m in 1:length(file[,1])) 
{
...
#' @usage immune_phylograms(immune_phylo_list_file)
#' @param immune_phylo_list_file A text file containing a list of sample files, one file name per line. Each file contains a matrix with proportion of immune cells (inferred by CIBERSORT) of all immune cell types in each tumor region and normal. 
#' @example immune_phylograms(immune_phylo_list.txt)
#' @export
immune_phylograms=function(immune_phylo_list_file){
file <- read.table(immune_phylo_list_file)
for (m in 1:length(file[,1])) 
{
...
#' @example z_score_calculations(S1_RNA_expresion_matrix)
#' @export 
